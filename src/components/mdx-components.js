    ...
    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 my-6">
      {items.map((item, i) => (
        <Card key={i} className="p-4">
          <p className="text-xs text-highlight font-bold uppercase tracking-widest mb-3">{item.label}</p>
          <audio controls className="w-full rounded-lg hue-rotate-80 brightness-105 contrast-150">
          ...
          <thead className="bg-white/5 border-b border-white/10">
            <tr>
              {headers.map((header, i) => (
                <th key={i} className="px-6 py-4 text-highlight font-bold uppercase text-xs tracking-widest">
                  {header}
                </th>
              ))}
            </tr>
